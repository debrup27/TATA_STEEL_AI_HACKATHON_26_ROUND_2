...
export default function UserPill({
  ...
  const content = (
    <div className="relative flex items-center group/profile">
      <div className={`${className} rounded-full bg-[#1b253c] flex items-center justify-center cursor-default shadow-sm`}>
        <User className="w-4 h-4 text-white" />
      </div>

      <div
        className={`absolute ${align === "left" ? "left-0" : "right-0"} top-full pt-5 min-w-[140px] z-50
          opacity-0 invisible pointer-events-none
          group-hover/profile:opacity-100 group-hover/profile:visible group-hover/profile:pointer-events-auto
